...
if (isset($_POST['action']) && $_POST['action'] == 'accept' && isset($_POST['id'])) {
  $id = intval($_POST['id']);
  $sql = "UPDATE bookings SET status = 'Accepted' WHERE id = $id";
  if (mysqli_query($data, $sql)) {
      // Success: Show checkmark on the table row
      echo "<script>
              document.getElementById('accept-btn-' + " . $id . ").innerHTML = '✔';
              document.getElementById('accept-btn-' + " . $id . ").disabled = true;
            </script>";
  } else {
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
  $id = intval($_GET['id']);
  $sql = "DELETE FROM bookings WHERE id = $id"; // Ensure correct table name
  if (mysqli_query($data, $sql)) {
      // Success: Show success notification and reload the page
      echo "<script>
              Swal.fire({
                  title: 'Deleted!',
      ...
      echo "<script>
              Swal.fire({
                  ...
                  location.reload(); // Reload the page even on error
              });
            </script>";
  }
  exit();
  ...
  <nav>
    ...
    <button id="closeMenu" class="close-btn">X</button>
     ...
     <div class="user-profile">
      ...
      <a href="#" onclick="location.reload()"><img src="logo.jpg" alt="logo1" style="height: 40px;"></a>
    ...
    <div class="content">
      <h2 style="text-align:center; margin-bottom: 5px;">Admin Dashboard - Bookings</h2>
        ...
        <tbody>
          ...
          <?php
            // Check if there are any bookings and display them
            if (mysqli_num_rows($result) > 0) {
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                if ($row['status'] == 'Accepted') {
                  echo "<td><button class='accept-btn' id='accept-btn-" . $row['id'] . "' disabled>✔</button></td>";  // Disabled if already accepted
                } else {
                  echo "<td>
                  ...
                  echo "<td><button class='delete-btn' onclick='deleteBooking(" . $row['id'] . ")'>Delete</button></td>"; // Delete button
                  echo "</tr>";
                  }
                } else {
  ...
  <script>
    // Accept booking and show SweetAlert
      ...
      if (result.isConfirmed) {
        window.location.href = 'adminbooking.php?action=accept&id=' + id;
      }
    });
  }
      // Delete booking and show SweetAlert
      ...
      if (result.isConfirmed) {
        window.location.href = 'adminbooking.php?action=delete&id= ' + id;
      }
    });
  }
